...
# @Last Modified time: 2022-05-14 18:57:07
...
def addCnsCurve(parent, name, centers, degree=1):
    ...
    aboutPy.gear_curvecns_op(node, centers)
...
def average_curve(crv,
    ...
    if shapes and len(shapes) >= average:
        shapes_by_distance = aboutTransform.get_closes_transform(crv, shapes)
        bst = []
        bst_filtered = []
        bst_temp = []
        weights = []
        ...
        if avg_rot:
            aboutTransform.interpolate_rotation(crv, bst, blends)
        if avg_scl:
            aboutTransform.interpolate_scale(crv, bst, blends)
        if avg_shape:
