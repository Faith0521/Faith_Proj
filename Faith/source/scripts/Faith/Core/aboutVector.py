...
# @Last Modified time: 2022-05-14 18:56:34
...
def getNormalVectorFromMod(prAxis , secAxis):
    if prAxis == "X" : prAxis = datatypes.Vector(1 , 0 , 0)
    if prAxis == "Y" : prAxis = datatypes.Vector(0 , 1 , 0)
    if prAxis == "Z": prAxis = datatypes.Vector(0, 0, 1)
    if prAxis == "-X" : prAxis = datatypes.Vector(-1 , 0 , 0)
    if prAxis == "-Y" : prAxis = datatypes.Vector(0 , -1 , 0)
    if prAxis == "-Z": prAxis = datatypes.Vector(0, 0, -1)

    if secAxis == "X" : secAxis = datatypes.Vector(1 , 0 , 0)
    if secAxis == "Y" : secAxis = datatypes.Vector(0 , 1 , 0)
    if secAxis == "Z": secAxis = datatypes.Vector(0, 0, 1)
    if secAxis == "-X" : secAxis = datatypes.Vector(-1 , 0 , 0)
    if secAxis == "-Y" : secAxis = datatypes.Vector(0 , -1 , 0)
    if secAxis == "-Z": secAxis = datatypes.Vector(0, 0, -1)

    normalAxis = prAxis ^ secAxis

    return normalAxis

def linstep(first, second, value):

    blend = float(first - value) / float(first - second)

    if value < min(first,second):
        blend = 0.0
    if value > max(first,second):
        blend = 1.0
