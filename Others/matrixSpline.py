...
# @Last Modified time: 2022-06-08 17:33:34
"""
...
def spline(pts, param, degree):
    ...
    """
    count = len(pts)
    knots = get_knots(count, degree)
    ws = getSpline_ws(knots, degree, param, count)
    points = [w*p for w,p in zip(ws, pts)]
    p = points.pop(0)
    return sum(points, p)

def spline_2(ps, param):
    span_num = len(ps) - 3
...
def tail_compute(cms, count, direction):
    """
    ...
    jps =  [spline_2(cps, i*1.0/(count-1)) for i in range(count)]
    return jps
